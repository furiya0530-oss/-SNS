...
■ 銘柄の設計(=期待される判定結果)
    1001 ヨコヨコ機械      : 値幅大・実体小が毎日続く   → ヨコヨコ条件で最上位に来るはず
    1002 ボラ電子        : 同上(やや弱い)           → ヨコヨコ条件で上位に来るはず
    1003 きわどい商事    : 該当日が約半分            → 初期値60%では落ちる(境界テスト用)
    2001 上昇トレンド薬品: 値幅は大きいが実体も大きい → ヨコヨコ条件では落ちるはず
    2002 下落トレンド銀行: 同上(下向き)             → ヨコヨコ条件では落ちるはず
    3001 静かなインフラ  : そもそも値幅が小さい       → ヨコヨコ条件では落ちるはず
    3002 出来高急増テック: 普段は静か→最終日に急騰   → 標準条件(出来高急増・ギャップ)だけ光る
    4001 低流動性テスト  : レンジ型だが売買代金が小さい → ヨコヨコ度は高いが売買代金条件で落ちる
"""
...
def profile_range_strong(i):
    """1001 ヨコヨコ機械:値幅は大きい(3.5〜5.5%)が実体はごく小さい(±0.6%以内)。
    ほぼ毎日がヨコヨコ条件の“該当日”になるはずの、いちばん分かりやすい正解銘柄。"""
    return (
...
def profile_uptrend(i):
    ...
    「値幅が大きい=ヨコヨコ条件に該当」ではないことを確認するための対照銘柄。"""
    return (
...
def profile_quiet(i):
    ...
    「始値と終値が同じ=ヨコヨコ条件に該当」ではないことを確認するための対照銘柄。"""
    return (
...
def profile_illiquid(i):
    """4001 低流動性テスト:値動きの形は 1001 と同じヨコヨコ型。
    ただし株価も出来高も小さいので、売買代金(終値×出来高)が10億円に遠く届かない。
    「ヨコヨコ度は高いが、標準条件では弾かれる」ケースの確認用。"""
    return (
        rng.uniform(-0.004, 0.004),
...
STOCKS = [
    ("1001", "ヨコヨコ機械",       2500, 900_000,   profile_range_strong),
