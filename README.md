...
清掃スタッフが現場のスマホから使うため、レスポンシブとタップ操作を優先しています。

- ボタンのタップ領域はスマホで **44px 以上** (`min-h-11`)、画面が広いときは詰める
  (`src/lib/ui.ts` にクラスを集約)
- 備品の数量増減ボタンとチェックリストの OK/不足/破損 は特に大きめ
- 通信中はスピナー、失敗時は `role="alert"` のメッセージを出す
  (`src/components/Feedback.tsx` に集約)
- キーボード操作向けにフォーカスの輪郭を表示し、`prefers-reduced-motion`
  では回転アニメーションを止める
