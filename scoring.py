...
■ このファイルの役割
    screening.py が出した「標準条件の充足状況」と「独自条件のスコア」を
    1つの総合スコアにまとめ、高い順に並べた一覧表を作ります。

■ 設計の考え方:先にものさしを揃える
    標準条件は「5つのうち何個満たしたか(0〜5個)」、
    独自条件は「該当日の割合(0〜1)」で、そのままでは単位が違います。
...
# 重みの初期値。独自条件がこのツールの主眼なので、やや大きめに配分しています。
# (画面のスライダーで変更できます)
...
def combine_scores(
    ...
    merged["独自スコア"] = merged["独自スコア"].fillna(0)
    merged["独自条件クリア"] = merged["独自条件クリア"].fillna(False).astype(bool)
...
def build_ranking(data, params):
    ...
    「標準条件を計算 → 独自条件を計算 → 合体して並べ替え」の順番を意識せずに済みます。
